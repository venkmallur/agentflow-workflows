# AgentFlow Workflows — Technical Write-Up

## Schema Reasoning

### Why This Schema Shape

The data model follows a strict ownership hierarchy: **Organization → Members → Workflows → Steps/Triggers → Runs → Step Runs**. Every piece of data traces back to an organization, which is the core multi-tenancy boundary.

**Key design decisions:**

1. **UUID primary keys everywhere** — prevents ID enumeration attacks and makes cross-org ID guessing infeasible even at the database level.

2. **Enums instead of strings** — `org_role`, `step_type`, `trigger_type`, `run_status`, and `step_run_status` are PostgreSQL enums. This gives us database-level validation without any application code, preventing invalid states from ever being stored.

3. **JSONB config columns** — `workflow_steps.config` and `workflow_triggers.config` use JSONB rather than separate tables per step/trigger type. This makes the schema extensible (adding a new step type doesn't require a migration) while keeping queries simple. The tradeoff is we lose column-level type safety, which we compensate for with validation in the Action handlers.

4. **`org_members` as a junction table** — a user can belong to multiple organizations. We deliberately do NOT store the org role in a JWT claim because a user's role varies per org. Instead, permissions check the `org_members` table dynamically via Hasura relationship traversal.

5. **`step_runs` separate from `workflow_steps`** — a step definition (what to do) is separate from a step execution (what happened). This means re-running a workflow creates new `step_run` records without touching the step definitions, and you get a full audit trail.

6. **`watched_table_events`** — a dedicated table for the database event trigger. An insert here fires a Hasura Event Trigger that evaluates whether any workflow should be triggered. This is a clean separation between "something happened in the database" and "a workflow should run."
...
**Example — `workflows` table select permission:**
```yaml
filter:
  organization:
    org_members:
...
- This is enforced at the **database query level** — Hasura appends WHERE clauses to every SQL query
...
3. **Approval gate authorization:** When a workflow is paused at an `approval_gate` step, the `approveStep` Action handler must verify the approver is an owner or editor **in the workflow's org** before resuming. This is a mid-execution decision — the workflow is already running, and a simple row-level permission can't express "this user may advance this specific run."

**Why this can't be Layer 1:** Hasura permissions operate on table rows — they can check "can this user see this step_run?" but not "should this user be allowed to resume the workflow engine?" The approval is an imperative action (update step_run + resume execution), not a simple row update. The handler fetches the org membership, checks the role, and only then updates the state and resumes step execution.

```typescript
// In approveStep handler — Layer 2 enforcement
...
3. The handler:
   ...
   c. Updates `step_run`: status → `'approved'`, sets `approved_by`, `approved_at`
   d. Updates `workflow_run`: status → `'running'`
...
- The subscription provides the bridge — the frontend sees the pause in real-time and presents the approval button
...
```
Step 1: llm_call        → pending → running → completed ✅
Step 2: conditional     → pending → running → completed ✅
Step 3: http_request    → pending → running → completed ✅
Step 4: approval_gate   → pending → running → paused_approval ⏸️  ← UI shows "Approve" button
                         (user clicks Approve)
Step 4: approval_gate   → approved ✅
Step 5: db_write        → pending → running → completed ✅
Run: completed ✅
```

Each status transition is a separate database update, and Hasura's live query subscription detects the change and pushes it to all connected clients — no polling, no page refresh.
